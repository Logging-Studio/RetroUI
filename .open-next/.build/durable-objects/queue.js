...
// node_modules/.pnpm/@opennextjs+cloudflare@1.19.5_next@16.2.2_@babel+core@7.29.0_@opentelemetry+api@1.9.0_babel-p_mqdr3ceixyfqll27yz55r7ykza/node_modules/@opennextjs/cloudflare/dist/api/durable-objects/queue.js
...
var DOQueueHandler = class extends DurableObject {
    ...
    try {
      ...
      response = await this.service.fetch(`${protocol}://${host}${url}`, {
        ...
        headers: {
          ...
          "x-prerender-revalidate": "599d0061bf3b8b6790332bee5196e72f",
          "x-isr": "1"
        },
  ...
  async addToFailedState(msg) {
    ...
    if (!this.disableSQLite) {
      this.sql.exec("INSERT OR REPLACE INTO failed_state (id, data, buildId) VALUES (?, ?, ?)", msg.MessageDeduplicationId, JSON.stringify(updatedFailedState), "mHVpVJwN7Hj_3Y7v32R1s");
    }
    await this.addAlarm();
  }
  async addAlarm() {
  ...
  async initState() {
    ...
    this.sql.exec("DELETE FROM failed_state WHERE buildId != ?", "mHVpVJwN7Hj_3Y7v32R1s");
    this.sql.exec("DELETE FROM sync WHERE buildId != ?", "mHVpVJwN7Hj_3Y7v32R1s");

...
function transformPreviewContent(content: string): string {
  ...
  // Note: Data attribute transformations (data-state -> data-open/data-closed)
  // are component-specific and may need manual review
  // This script will flag files that contain these patterns

  return transformed;
}

function needsManualReview(content: string): boolean {
...
function generateBaseUIPreviews() {
  ...
  const baseuiPreviewDir = path.join(previewDir, "baseui");

  // Create baseui directory if it doesn't exist
  if (!fs.existsSync(baseuiPreviewDir)) {
    fs.mkdirSync(baseuiPreviewDir, { recursive: true });
  }
